...
'''
class KDManager2(nn.Module):
    def __init__(self, args):
        super(KDManager2, self).__init__()
        self.args = args
    ...
    def forward(self, sample, mode):
        head, tail = self.EntityEmbeddingExtract(self.origin_entity_embedding, sample, mode)
        relation = self.RelationEmbeddingExtract(self.origin_relation_embedding, sample)
        return head, relation, tail, relation
